...
public:
		...
		: ActionPacket( InData, Length, bNeedSwap )
	{
		...
		if( bNeedSwap ) SwapEndian32( &ServerInfo );
		index += 4;
		memcpy(&Area,&Data[index],4);
		if( bNeedSwap ) SwapEndian32( &Area );
		index += 4;
		memcpy(&StepNum,&Data[index],2);
		if( bNeedSwap ) SwapEndian16( &StepNum );
		index += 2;

		for(INT i=0;i<StepNum;i++)
		{
			memcpy(&Locations[i].X,&Data[index],2);
			if( bNeedSwap ) SwapEndian16( &Locations[i].X );
			index += 2;
			memcpy(&Locations[i].Y,&Data[index],2);
			if( bNeedSwap ) SwapEndian16( &Locations[i].Y );
			index += 2;
		}

		memcpy(&FaceDir,&Data[index],2);
		if( bNeedSwap ) SwapEndian16( &FaceDir );
		index += 2;
		memcpy(&TimeStamp,&Data[index],4);
		if( bNeedSwap ) SwapEndian32( &TimeStamp );
		index += 4;
	}
...
int WriteBinData(char *function, char *buf, int len)
{
	...
	{
		//if( MovePacket::IsMovePacket( (BYTE*)buf, len ) )
		{
			...
			case EActionType_Move:
				...
				wsprintfA(temp, "\r\n[Move] SeverInfo: %x, Area:%x, StepNum:%x, FaceDir:%x, TimeStamp:%x", 
